...
<head>
  ...
  <style>
    .sort-icons { display: flex; align-items: center; gap: 2px; cursor: pointer; }
    .arrow { font-size: 1rem; opacity: 0.4; font-weight: 400; color: white; transition: all 0.3s ease; }
    .arrow.active { opacity: 1; font-weight: 700; transform: scale(1.1); }
    .header-active { font-weight: 700; color: white; }
  </style>
</head>
      ...
      <tbody id="hasil">
        @foreach ($mahasiswas as $m)
          <tr class="border-b hover:bg-gray-100 transition-colors duration-300">
            ...
            <td class="py-3 px-6">{{ $m->prodi->fakultas->nama_fakultas ?? '-' }}</td>
            <td class="py-3 px-6">
              {{ $m->prodi->nama_prodi }} ({{ $m->prodi->kode_prodi }})
            </td>
          ...
          data.forEach(m => {
            hasil.innerHTML += `
              <tr class="border-b hover:bg-gray-100 transition-colors duration-300">
                ...
                <td class="py-3 px-6">${m.nama}</td>
                <td class="py-3 px-6">${m.prodi?.fakultas?.nama_fakultas ?? '-'}</td>
                <td class="py-3 px-6">${m.prodi?.nama_prodi ?? '-'} (${m.prodi?.kode_prodi ?? '-'})</td>
                @if(Auth::user()->role === 'admin')
                <td class="py-3 px-6 flex gap-2">
