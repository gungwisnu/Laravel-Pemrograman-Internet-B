...
<head>
  <meta charset="UTF-8">
  <title>Konfirmasi Hapus Prodi</title>
  ...
  <div class="bg-white shadow-xl rounded-xl p-8 max-w-md w-full">
    ...
    <div class="mb-6">
      <table class="w-full text-left border border-gray-200 rounded-md">
        <tr><th class="p-2 bg-gray-50 border-b">ID</th><td class="p-2 border-b">{{ $prodi->id }}</td></tr>
        <tr><th class="p-2 bg-gray-50 border-b">Kode</th><td class="p-2 border-b">{{ $prodi->kode_prodi }}</td></tr>
        <tr><th class="p-2 bg-gray-50 border-b">Nama</th><td class="p-2 border-b">{{ $prodi->nama_prodi }}</td></tr>
        <tr><th class="p-2 bg-gray-50">Fakultas</th><td class="p-2">{{ $prodi->fakultas?->nama_fakultas ?? '-' }}</td></tr>
      </table>
    </div>

    <form method="POST" action="{{ route('prodi.destroy', $prodi->id) }}" class="flex justify-center gap-4">
      @csrf
      @method('DELETE')
