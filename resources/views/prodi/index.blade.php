...
<body class="bg-gray-100 font-sans">
  ...
  <div class="container mx-auto px-4 py-6">
    <h2 class="text-3xl font-bold text-center mb-6">Daftar Program Studi</h2>
    ...
    <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
      <thead class="bg-gradient-to-r from-blue-400 to-blue-600 text-white">
        <tr>
          ...
          <th class="py-3 px-6 text-left">Fakultas</th>
          @if(Auth::user()->role === 'admin')
      ...
      <tbody id="hasil">
        @foreach ($prodi as $p)
          <tr class="border-b hover:bg-gray-100 transition-colors duration-300">
            ...
            <td class="py-3 px-6">{{ $p->kode_prodi }}</td>
            <td class="py-3 px-6">{{ $p->nama_prodi }}</td>
            <td class="py-3 px-6">{{ $p->fakultas?->nama_fakultas ?? '-' }}</td>

            @if(Auth::user()->role === 'admin')
              <td class="py-3 px-6 flex gap-2">
                <a href="{{ route('prodi.edit', $p->id) }}" class="text-blue-500 hover:text-blue-700">Edit</a> | 
                <a href="{{ route('prodi.confirmDelete', $p->id) }}" class="text-red-500 hover:text-red-700">Hapus</a>
              </td>
            @endif
          </tr>
        @endforeach
